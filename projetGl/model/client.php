<?php
	...
	class Client {
		...
		// additional function
			...
			function getContact() {
				if (isConnectMySql()) {
                    $sql = 'select c.id as contact, p.id, p.nom, p.prenom, p.mail, p.telephone from projetGL_personne p join projetGL_contact c on p.id = c.personne where etat = 1 and c.client = ' . sanitize_string($this->_id) . ';';
                    $result = $_SESSION["link"]->query($sql);
					if ($result->num_rows == 0){
						return null;
					}
                        ...
                        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                            $retVal[$i] = new Contact($row["contact"], new Personne($row["id"], $row["nom"], $row["prenom"], $row["mail"], $row["telephone"]));
                            $i++;
                        }
                        return $retVal;
					}
